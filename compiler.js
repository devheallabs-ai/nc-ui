...
class CodeGenerator {
  constructor(ast, options) {
    ...
    this.accent = ast.meta.accent || '#2b90ff';
  ...
  _node(n, ctx) {
    ...
    switch (n.type) {
      ...
      case 'Link':       return `<a href="${this._safeUrl(n.href || '#')}" class="ncui-link">${this._interpolateHTML(n.text, scope)}</a>`;
      case 'Image':      return this._image(n);
      case 'Video':      return `<video src="${this._safeUrl(n.src)}" controls class="ncui-video"></video>`;
  ...
  _markdownInline(text) {
    return String(text || '')
      ...
      .replace(/\*([^*]+)\*/g, '<em>$1</em>')
      .replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, label, url) => `<a href="${this._safeUrl(url)}">${label}</a>`);
  }
  ...
  _button(n, ctx) {
    ...
    if (n.href) {
      if (this._hasRoutes && n.href.startsWith('/')) {
        return `<a href="${this._safeUrl(n.href)}" class="${cls}" data-ncui-link="${this._safeUrl(n.href)}"${attrStr}>${label}</a>`;
      }
      return `<a href="${this._safeUrl(n.href)}" class="${cls}"${attrStr}>${label}</a>`;
    }
  ...
  _image(n) {
    ...
    return `<img src="${this._safeUrl(n.src)}" alt="${this._esc(n.alt || '')}" class="${cls.join(' ')}" loading="lazy">`;
  }
...
${this._scriptTag()}
  ...
  _renderProtectedRouteShell(access) {
    ...
    const redirectHtml = access.redirect
      ? `<p class="ncui-text"><a class="ncui-link" href="${this._safeUrl(access.redirect)}">Continue to ${this._esc(access.redirect)}</a></p>`
      : '';
