...
|-------------|----------------|-------|
| NC engine (`nc` binary) | **v1.0.0** | Required for native compiler (`nc ui build`) |
| Node.js | 16.x | Required for legacy JS compiler only (`npm install -g nc-ui`) |

The native compiler path (`nc ui build`) is zero-dependency and recommended for all new projects.
The Node.js path is retained for web-based tooling and legacy workflows.
...
# Build your first page natively (Zero dependencies)
echo 'page "Hello"
theme "dark"
...
NC UI is now **native**. The recommended way to use it is via the `nc` executable, which has **zero external dependencies** (no Node.js required).

### Native NC Engine (Recommended)
...
The original JavaScript compiler is still available for web-based tools and legacy workflows. It runs on Node.js 16+.
...
# Now use it anywhere:
ncui build mysite.ncui
ncui serve mysite.ncui
```

The JavaScript CLI is intentionally plain-text and log-friendly. For regression coverage, `tests/test_cli.js` now verifies repo-relative execution and `NO_COLOR` / `NC_NO_ANIM` compatibility for build output.

### Standalone (no install)

```bash
git clone https://github.com/devheal-labs/nc-ui.git
cd nc-ui
...
```bash
# macOS / Linux
./start.sh build mysite.ncui
./start.sh serve mysite.ncui
./start.sh playground
./start.sh examples

# Windows
start.bat build mysite.ncui
start.bat serve mysite.ncui
...
```bash
docker build -t nc-ui .
docker run -p 3000:3000 nc-ui
```
...
```
nc-ui/
  compiler.js       — The NC UI compiler (parser + code generator)
  nc-ui.js          — Browser runtime (client-side compilation + API)
  cli.js            — Command-line interface (build, watch, serve)
  ...
  Dockerfile        — Docker container for the dev server
  examples/
